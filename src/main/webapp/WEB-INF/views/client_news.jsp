		...
		<div class="container-xxl py-5 hero-header">
          <div class="container my-5 py-5 px-lg-5">
			  ...
			  <table class="table animated fadeIn">
				...
				<tbody>
					...
					<tr>
						...
						<td><a href="news_detail?board_seq=${i.board_seq }&page=${pageMaker.cri.page}">${i.board_title }</a></td>
						<td>${i.board_date }</td>
					</tr>
					</c:forEach>
				</tbody>
            ...
            <div class="text-end content_margin4" id="admin_space">
				<a href="/news_write" class="btn btn-light py-sm-3 px-sm-5 rounded-pill animated slideInDown">글쓰기</a>
			</div>
          </div>
          ...
          <!-- PAGING -->
			<div class="pagination justify-content-center text-center">
				...
				<c:if test="${pageMaker.prev }">
					<li><a href='<c:url value="/news?page=${pageMaker.startPage-1 }"/>'>
						<i class="fa-chevron-left"></i></a></li>
				</c:if>
				
				<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
					<li><a class="page-link" href='<c:url value="/news?page=${pageNum }"/>'>
						<i class="fa-solid fa-1x">${pageNum }</i></a></li>
				</c:forEach>
				
				<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
					<li><a href='<c:url value="/news?page=${pageMaker.endPage+1 }"/>'>
