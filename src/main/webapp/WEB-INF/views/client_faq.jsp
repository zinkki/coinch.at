		...
		<div class="row g-5 py-5 content_margin2">
         ...
         <input type="hidden" name="coin_user_adminYN" id="coin_user_adminYN" value='<%= (String)session.getAttribute("check_admin") %>'>
      	...
      	<div class="col-12 text-center">
				...
				<tbody>
					...
					<tr>
						...
						<td><a href="faq_detail?board_seq=${i.board_seq }&page=${pageMaker.cri.page}">${i.board_title }</a></td>
						<td>${i.board_date }</td>
					</tr>
					</c:forEach>
				</tbody>
      	...
      	<div class="text-end content_margin4" id="admin_space">
			<a href="/faq_write" class="btn btn-light py-sm-3 px-sm-5 rounded-pill animated slideInDown">글쓰기</a>
		</div>
       ...
       <!-- PAGING -->
			<div class="pagination justify-content-center text-center">
				...
				<c:if test="${pageMaker.prev }">
					<li><a href='<c:url value="/faq?page=${pageMaker.startPage-1 }"/>'>
						<i class="fa-chevron-left"></i></a></li>
				</c:if>
				
				<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
					<li><a class="page-link" href='<c:url value="/faq?page=${pageNum }"/>'>
						<i class="fa-solid fa-1x">${pageNum }</i></a></li>
				</c:forEach>
				
				<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
					<li><a href='<c:url value="/faq?page=${pageMaker.endPage+1 }"/>'>
